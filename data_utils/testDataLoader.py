...
class testttDataLoader(Dataset):
    ...
    def _get_item(self, index):        
        ...
        newpointpath=os.path.join(self.scenepath,finalsource[0],finalsource[1],'newpointcloud')
        ...
        for idx in range(rangenum):
            ...
            point=np.load(os.path.join(newpointpath,str(idx+1+int(finalsource[2]))+'.npy'))
            randomlist=np.random.choice(np.asarray(point).shape[0],size=(self.num))
            pointcloud[idx,:,:]=point[randomlist]
